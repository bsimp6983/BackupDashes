...
function LoadGridData() {
    ...
    $.post("Service.ashx?op=loadgriddata", "", function(data) {
        $('#_loading').remove();
        ...
        $.each(data, function() {
            ...
            row = "<tr>";
            row += "<td class=\"col1\">" + (new Date(this.EventStart)).format("MM/dd/yyyy h:mm t") + "</td>";
            row += "<td class=\"col2\">" + secondToString(this.Minutes * 60) + "</td>";
            if ($.trim(this.ReasonCode).length > 0) {
                row += "<td class=\"col3\"><a href=\"javascript:AssignReasonCode(" + this.ID + ")\">" + this.ReasonCode + "</a></td>";
            }
            else {
                $("#tipBox").css("display", "block");
                row += "<td class=\"col3\"><input type='button' id='assign_reason_code' value='' onclick='AssignReasonCode(" + this.ID + ")' /></td>";
            }
            row += "</tr>";
            tb.append(row);
            rownum++;
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            $('#btnSave').click(function() {
                ...
                $.post("Service.ashx?op=save", "id=" + $('#id').val() + "&reasonId=" + reasonid + "&reasonCode=" + UrlEncode(reason) + "&comment=" + UrlEncode(comment) + "&startdatetime=" + UrlEncode(startdatetime) + "&minutes=" + UrlEncode(minutes) + "&line=" + UrlEncode(line), function(data) {
                    if (data.toString().toLowerCase() == 'true') {
