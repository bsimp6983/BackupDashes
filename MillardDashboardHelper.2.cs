...
namespace DCSDemoData
{
    public partial class MillardDashboardHelper
    {
        ...
        public static Dictionary<string, decimal> getLast12hrsCaseCounts(DateTime clientTime, out List<CaseCount> caseCounts, string line, bool increments)
        {
            Dictionary<string, decimal> results = new Dictionary<string, decimal>();

            List<CaseCount> list = new List<CaseCount>();

            foreach (var item in getGraphCaseCounts(clientTime, line, increments))
            {
                results.Add(convertTo12hrString(item.Key.EventStop.Value.Hour), (int)item.Value);
                list.Add(item.Key);
            }
        ...
        public static List<CaseCount> getLast12hrsCaseCountList(DateTime? clientTime, string line)
        {
            List<CaseCount> list = new List<CaseCount>();
        ...
        public static List<CaseCount> getLast12hrsCaseCountList(DateTime? startTime, DateTime? endTime, string line)
        {
            List<CaseCount> list = new List<CaseCount>();

            Dictionary<CaseCount, decimal> cases = getGraphCaseCounts(startTime, endTime, line);
        ...
        public static Dictionary<CaseCount, decimal> getGraphCaseCounts(DateTime clientTime, string line, bool increments = true)
        {
            DateTime d = clientTime.AddHours(-12);
            DateTime? startTime = new DateTime(d.Year, d.Month, d.Day, d.Hour, 0, 0);
            DateTime? endTime = new DateTime(clientTime.AddHours(-1).Year, clientTime.AddHours(-1).Month, clientTime.AddHours(-1).Day, clientTime.AddHours(-1).Hour, 59, 59);
